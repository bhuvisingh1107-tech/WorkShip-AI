...
const KnowledgePageInner = () => {
  ...
  useEffect(() => {
    ...
    fetcher<DocumentsResponse>(`/api/documents?${params}`)
    ...
    <section className="space-y-6 px-4 py-8 sm:px-6 lg:px-8 lg:py-10">
      ...
      <form className="flex gap-3" onSubmit={submit}>
        <div className="relative flex-1">
          ...
          <input
            className="w-full rounded-lg border border-[#DCD6FF] py-2.5 pl-9 pr-3 text-sm outline-none focus:border-[#A5B4FC]"
            onChange={(event) => setSearch(event.target.value)}
            placeholder="Search enterprise knowledge…"
            value={search}
          />
        </div>
        <button className="rounded-lg bg-[#7C6CE7] px-4 text-sm font-medium text-white shadow-sm shadow-[B8B5FF]/50" type="submit">Search</button>
      </form>
      ...
      {!isLoading && !error && (
        <>
          <p className="text-sm text-slate-500">{total} documents found</p>
          {documents.length > 0 && (
            <>
              <div className="rounded-xl border border-[#E9E5FF] bg-white p-4 shadow-sm shadow-[B8B5FF]/10">
                <p className="text-sm font-semibold text-[#312E81]">Knowledge base distribution</p>
                <div className="mt-3 flex h-3 overflow-hidden rounded-full bg-[#F5F3FF]">
                  {Object.entries(
                    documents.reduce<Record<string, number>>((counts, item) => ({
                      ...counts,
                      [item.category ?? "Other"]: (counts[item.category ?? "Other"] ?? 0) + 1,
                    }), {})
              ...
              {documents.length === 0 ? (
                <div className="rounded-xl border border-dashed border-[#DCD6FF] p-10 text-center text-sm text-slate-500">
                  No documents match your search.
                </div>
              ) : (
                <div className="gap-4 lg:grid-cols-2">
                  {documents.map(
                    (document) => (
                      <article
                        className="rounded-xl border border-[#E9E5FF] bg-white p-5 shadow-sm shadow-[B8B5FF]/10"
                        key={document.id}
                      >
                        <div className="flex items-start gap-3">
                          <BookOpen className="mt-0.5 size-5 shrink-0 text-[#818CF8]" />
                          <div>
                            <h2 className="font-semibold text-[#312E81]">{document.title}</h2>
                            <p className="mt-1 text-xs text-slate-500">
                              {document.category ?? "Enterprise knowledge"} · {readingTime(
                                document.content
                              )} min read
                            </p>
                          </div>
                        </div>
                        <p className="mt-4 text-sm leading-6 text-slate-600">{document.summary ?? document.content.slice(0, 220)}</p>
                        <div className="mt-4 flex flex-wrap gap-2">
                          {document.tags?.map(
                            (tag) => (
                              <span
                                className="rounded-full bg-[#CFFAFE] px-2 py-1 text-xs text-[#315E70]"
                                key={tag}
                              >
                                {tag}
                              </span>
...
export default function KnowledgePage() {
  return <RequireAuth><KnowledgePageInner /></RequireAuth>;
}