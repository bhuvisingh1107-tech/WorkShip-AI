      ...
      <header className="flex h-16 items-center justify-between border-b border-[#E9E5FF] bg-white/90 px-4 backdrop-blur dark:border-indigo-800 dark:bg-slate-900/90 sm:px-6">
        <div className="flex items-center gap-3">
          <button
            ...
            onClick={() => { /* TODO: implement sidebar toggle */ }}
            type="button"
          >
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            disabled={loading}
          >
            {loading ? <span className="inline-flex h-4 w-4 items-center justify-center rounded-full border border-slate-200 animate-spin">Loading</span> : <LogOut className="size-4" />}
          </button>
        </div>
      </header>
      ...
      <main className="pt-8">
        <section className="space-y-6 px-4 py-8 sm:px-6 lg:px-8 lg:py-10">
          <div className="flex items-start justify-between gap-4 rounded-2xl bg-gradient-to-br from-[#C4B5FD] via-[#B8B5FF] to-[#CFFAFE] px-6 py-7 text-[#312E81]">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[#5B54A6]">Executive view</p>
              <h1 className="mt-2 text-2xl font-semibold">Company Sales</h1>
              <p className="mt-2 text-sm text-[#5B54A6]">Revenue performance, client health, and recommended commercial actions.</p>
            </div>
            <button onClick={handleLogout} className="inline-flex items-center gap-2 rounded-lg bg-white/70 px-3 py-2 text-xs font-medium text-[#312E81]">
              <LogOut className="size-4" />Logout
            </button>
          </div>
          <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            <Metric label="Total clients" value="42" icon={Users} tone="[#CFFAFE]" />
            <Metric label="Active clients" value="36" icon={Users} tone="[#A7F3D0]" />
            <Metric label="Revenue this year" value="$4.1M" icon={TrendingUp} tone="[#FDE68A]" />
            <Metric label="Growth" value="+28%" icon={TrendingUp} tone="[#FBCFE8]" />
          </div>
          <div className="grid gap-5 lg:grid-cols-3">
            <div className="rounded-xl border border-[#E9E5FF] bg-white p-5 shadow-sm shadow-[B8B5FF]/10 lg:col-span-2">
              <div className="flex items-center justify-between">
                <h2 className="font-semibold text-[#312E81]">Five year revenue trend</h2>
                <span className="rounded-full bg-[#A7F3D0] px-2 py-1 text-xs font-medium text-[#246B57]">+28% growth</span>
              </div>
              <svg aria-label="Five year revenue trend" className="mt-5 h-48 w-full" viewBox="0 0 500 180">
                <defs>
                ...
                <path d="M30 140 L145 105 L260 70 L375 110 L470 45 L470 155 L30 155 Z" fill="url(#sales-area)" />
                <path d="M30 140 L145 105 L260 70 L375 110 L470 45" fill="none" stroke="#7C6CE7" strokeWidth="4" />
            ...
            <div className="rounded-xl border border-[#E9E5FF] bg-white p-5">
              ...
              <ul className="mt-4 space-y-4 text-sm text-slate-700">
                ...
                <li className="flex gap-2">
                  <AlertCircle className="size-4 shrink-0 text-[#D977A7]" />
                  3 clients require follow-up
                </li>
                <li className="flex gap-2">
          ...
          <div className="grid gap-5 lg:grid-cols-2">
            <div className="rounded-xl border border-[#E9E5FF] bg-white p-5 shadow-sm shadow-[B8B5FF]/10">
              <h2 className="font-semibold text-[#312E81]">Top client performance</h2>
            ...
            <div className="rounded-xl border border-[#E9E5FF] bg-white p-5">
              <h2 className="font-semibold text-[#312E81]">Client distribution</h2>
          ...
          <div className="rounded-xl border border-[#E9E5FF] bg-white p-5">
            ...
            <div className="mt-4 overflow-x-auto">
              <table className="w-full min-w-[700px] text-left text-sm">
                ...
                <tbody>
                  {clients.map((client) => (
                    <tr className="border-b border-slate-100 last:border-0" key={client[0]}>
                      {client.map((value, index) => (
                        ...
                        >
                          {index === 3 ? (
                            <span className="rounded-full bg-[#F5F3FF] px-2 py-1 text-xs">{value}</span>
                          ) : index === 5 ? (
                            <span
                              className={value === "Up"
                                ? "text-emerald-600"
          ...
          <div className="grid gap-4 md:grid-cols-3">{[
            ...
            >
              ...
              <p className="mt-3 text-sm font-medium text-[#312E81]">{suggestion}</p>
            </div>
          ))}</div>
        </section>
      </main>
...
export default function CompanySalesPage() {
  return <RequireAuth><CompanySalesPageInner /></RequireAuth>;
}