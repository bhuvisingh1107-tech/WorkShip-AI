...
function SalesDashboard({ onLogout }: { onLogout: () => void }) {
  return <section className="space-y-6 px-4 py-8 sm:px-6 lg:px-8 lg:py-10">
    ...
    <div className="grid gap-5 lg:grid-cols-3"><div className="rounded-xl border border-[#E9E5FF] bg-white p-5 shadow-sm shadow-[#B8B5FF]/10 lg:col-span-2"><div className="flex items-center justify-between"><h2 className="font-semibold text-[#312E81]">Five year revenue trend</h2><span className="rounded-full bg-[#A7F3D0] px-2 py-1 text-xs font-medium text-[#246B57]">+28% growth</span></div><svg aria-label="Five year revenue trend" className="mt-5 h-48 w-full" viewBox="0 0 500 180"><defs><linearGradient id="sales-area" x1="0" x2="0" y1="0" y2="1"><stop stopColor="#B8B5FF" stopOpacity=".55" /><stop offset="1" stopColor="#CFFAFE" stopOpacity=".1" /></linearGradient></defs><path d="M30 140 L145 105 L260 70 L375 110 L470 45 L470 155 L30 155 Z" fill="url(#sales-area)" /><path d="M30 140 L145 105 L260 70 L375 110 L470 45" fill="none" stroke="#7C6CE7" strokeWidth="4" />{trend.map((item, index) => <g key={item.year}><circle cx={[30,145,260,375,470][index]} cy={[140,105,70,110,45][index]} fill="#fff" r="5" stroke="#7C6CE7" strokeWidth="3" /><text fill="#64748B" fontSize="12" textAnchor="middle" x={[30,145,260,375,470][index]} y="175">{item.year}</text></g>)}</svg></div><div className="rounded-xl border border-[#E9E5FF] bg-[#F5F3FF] p-5"><h2 className="font-semibold text-[#312E81]">Sales alerts</h2><ul className="mt-4 space-y-4 text-sm text-slate-700"><li className="flex gap-2"><TrendingDown className="size-4 shrink-0 text-[#D977A7]" />Sales decreased 15% compared to previous year</li><li className="flex gap-2"><AlertCircle className="size-4 shrink-0 text-[#D977A7]" />3 clients require follow-up</li><li className="flex gap-2"><CalendarClock className="size-4 shrink-0 text-[#5B54A6]" />Renewal meetings recommended</li></ul></div></div>
    <div className="grid gap-5 lg:grid-cols-2"><div className="rounded-xl border border-[#E9E5FF] bg-white p-5 shadow-sm shadow-[#B8B5FF]/10"><h2 className="font-semibold text-[#312E81]">Top client performance</h2><div className="mt-4 space-y-3">{clients.slice(0,3).map((client, index) => <div key={client[0]}><div className="flex justify-between text-sm text-slate-600"><span>{client[0]}</span><span>{client[2]}</span></div><div className="mt-1 h-2 rounded-full bg-[#F5F3FF]"><div className="h-2 rounded-full bg-gradient-to-r from-[#A5B4FC] to-[#A7F3D0]" style={{width:`${[100,79,63][index]}%`}} /></div></div>)}</div></div><div className="rounded-xl border border-[#E9E5FF] bg-white p-5 shadow-sm shadow-[#B8B5FF]/10"><h2 className="font-semibold text-[#312E81]">Client distribution</h2><div className="mt-4 grid grid-cols-3 gap-3 text-center"><MiniStat label="Active" value="36" tone="bg-[#A7F3D0]" /><MiniStat label="At risk" value="3" tone="bg-[#FBCFE8]" /><MiniStat label="Renewals" value="7" tone="bg-[#FDE68A]" /></div></div></div>
...
function MiniStat({ label, value, tone }: { label: string; value: string; tone: string }) { return <div className={`rounded-xl ${tone} p-3`}><p className="text-lg font-semibold text-[#312E81]">{value}</p><p className="mt-1 text-xs text-[#5B54A6]">{label}</p></div>; }
