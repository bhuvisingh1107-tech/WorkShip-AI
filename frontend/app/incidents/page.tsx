...
export default function IncidentsPage() {
  ...
  async function simulate(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok) throw new Error("Unable to simulate the incident.");
      ...
      <div className="rounded-xl border border-slate-200 bg-white shadow-sm">
        <div className="border-b border-slate-200 px-5 py-5 sm:px-6">
          <div className="flex items-center gap-3">
            <div className="rounded-lg bg-slate-900 p-2 text-white"><AlertTriangle className="size-5" /></div>
        ...
        <form className="grid gap-4 border-b border-slate-200 p-5 sm:grid-cols-2 sm:p-6" onSubmit={simulate}>
          ...
          <div className="sm:col-span-2"><button className="inline-flex items-center gap-2 rounded-lg bg-slate-900 px-4 py-2.5 text-sm font-medium text-white disabled:bg-slate-400" disabled={isLoading} type="submit">{isLoading && <LoaderCircle className="size-4 animate-spin" />}{isLoading ? "Investigating…" : "Simulate incident"}</button></div>
        </form>
        ...
        <div className="space-y-5 p-5 sm:p-6">
          ...
          {result && <div className="space-y-5">
            <div className="rounded-lg bg-slate-50 p-4"><p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Simulated incident</p><p className="mt-1 font-semibold text-slate-950">{result.incident.title}</p><p className="mt-1 text-sm text-slate-600">{result.incident.severity} · {result.incident.status}</p></div>
